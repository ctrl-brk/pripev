...
if(document)
   ...
   var a07=new Array("EADGBE|EADGBE - Стандартный","DADGBE|DADGBE - Drop-D","DADGBD|DADGBD - Double drop-D","DADF#AD|DADF#AD - Открытая D",
                     "DADFAD|DADFAD - Открытая D минор","DADGAD|DADGAD - DADGAD","DADDAD|DADDAD - D modal","EADGCF|EADGCF - Квартовая",
                     "EBEG#BE|EBEAbBE - Открытая E","DGDGBE|DGDGBE - G6","DGDGBD|DGDGBD - Открытая G","DGDGA#D|DGDGBbD - G минор",
                     "EAEAC#E|EAEAC#E - Открытая A","EAEACE|EAEACE - A минор","CGCGAE|CGCGAE - C6","CGCGCE|CGCGCE - Открытая C",
                     "CGDGAD|CGDGAD - Нижняя C","Custom|Персональный");
...
function c46(e)
{
   ...
   if ( key == 'в' || key == 'В' ) togglePlayingHand();
   if ( key == 'б' || key == 'Б' ) toggleBassStrings();
   if ( key == 'п' || key == 'П' ) toggleInNn();
   if ( key == 'г' || key == 'Г' ) toggleSpelling();
   if ( key == 'т' || key == 'Т' ) toggleSharpFlat();
}
...
function a79()
{
   ...
   chord("Diminished","Rdim","<R>Ш","1,b3,b5");
   chord("Diminished","Rdim7","<R>Ш7","1,b3,b5,bb7");
   chord("Half Diminished","Rm7-5","<R>°7, <R>Ѕdim, <R>Ѕdim7, <R>m(b7), <R>minor7b5","1,b3,b5,b7");
...
function showFingerSetting( c25 )
{
   ...
   else alert("Ошибка: `infoType'.\nПожалуйста сообщите по адресу webmaster@pripev.ru.\ninfoType = "+b19+"\nСпасибо.");
...
{
   document.aspnetForm.infoText.value="Подождите...   ";
   c44(a43);
   for( i=1; i < 7; i++ )
      {
      a59(i,c43);
   ...
   if ( a54 != "" ) document.aspnetForm.infoText.value+="Обозначения: " + a54 + "\n";
   document.aspnetForm.infoText.value+="тона:        " + a43 + "\n" + "полутона:    ";
   c12(a43,a13);
...
function setTuning(a14)
{
   var pos=0;
   var note="";
   window.status="Строй: " + a14;
   if ( a14 == "Custom" )
      {
      window.status="Настройте струны с помощью кнопок [+] и [-]";
      return;
      }
...
function b69(a06)
{
   ...
   document.write("Строй:<br><select name=\"TuningList\" size=1 class='SelectControl'" + "onChange=\"setTuning(this.options[this.selectedIndex].value);\">");
   var pos=-1;
   ...
   document.write("<td><img src=\"dot_clear.gif\" width=15 height=1></td><td><span class=\"rb\"><acronym title=\"Аккорды для правши\">" +
                  "<input type=radio name=\"fb_player\" value=\"R\" onClick=\"fb_change('fb_player','R')\">Пра<b><u>в</u></b>ша</acronym><br><acronym title=\"Аккорды для левши\">"+
                  "<input type=radio name=\"fb_player\" value=\"L\" onClick=\"fb_change('fb_player','L')\">Ле<b><u>в</u></b>ша</acronym></span></td>"+
                  "<td><img src=\"dot_clear.gif\" width=15 height=1></td><td><span class=\"rb\"><acronym title=\"Показывать гриф с басовыми струнами вверху\">"+
                  "<input type=radio name=\"fb_bass\" value=\"top\" onClick=\"fb_change('fb_bass','top')\"><b><u>Б</u></b>асовые струны вверху</acronym><br>"+
                  "<acronym title=\"Показывать гриф с басовыми струнами внизу\"><input type=radio name=\"fb_bass\" value=\"bottom\" onClick=\"fb_change('fb_bass','bottom')\"><b><u>Б</u></b>асовые струны внизу</acronym></span></td>");
...
function b83(a06)
{
   document.write("<select name=\"Capo\" size=1 class='SelectControl' onChange=\"setCapo(this.options[this.selectedIndex].value);\">" +
                  "<option value=\"0\"><option value=\"1\">Капо на 1-ом ладу"+"<option value=\"2\">Капо на 2-ом ладу<option value=\"3\">Капо на 3-ем ладу");
   for ( var i=4; i<11; i++ ) document.write("<option value=" + i + ">Капо на " + i + "-ом ладу");
...
function b70(a37)
{
   document.write("<td><a href=\"Javascript:tuneUp(" + a37 + ");\" onMouseOver=\"imgOn('tuneup" + a37 + "','tuneup_'); window.status='Подтянуть струну на " + a37 + " тон'; return true;\" onMouseOut=\"imgOff('tuneup" + a37 + "','tuneup_'); window.status=''; return true;\"><img border=0 src=\"tuneup_off.gif\" width=20 height=20 name=\"tuneup" + a37 + "\" alt=\"Подтянуть струну " + a37 + "\"></a></td>");
}

function b73(a37)
{
   document.write("<td><img src=\"h" + a08[6-a37] + ".gif\" width=20 height=20 name=\"s" + a37 + "h\"alt=\"Настройка струны " + a37 + "\"></td>");
}

function b71(a37)
{
   document.write("<td><a href=\"Javascript:tuneDown(" + a37 + ");\" onMouseOver=\"imgOn('tunedown" + a37 + "','tunedown_');window.status='Отпустить струну на " + a37 + " тон';return true;\" onMouseOut=\"imgOff('tunedown" + a37 + "','tunedown_');window.status='';return true;\"><img border=0 src=\"tunedown_off.gif\" width=20 height=20 name=\"tunedown" + a37 + "\" alt=\"Отпустить струну " + a37 + "\"></a></td>");
}
...
function b78(a37,fret_no)
{
   ...
   document.write("\" width=18 height=18 name=\"s" + a37 + fret_no + "\" alt=\"Струна " + a37 + ", лад " + fret_no + "\"></span>");
...
function b81(a06)
{
   ...
   document.write("<td width=180><span id=\"rb_display_names\" class=\"rb\"><acronym title=\"Ноты: C, D, E, итд.\"><input type=radio name=\"displayNames\" value=\"nn\" onClick=\"switchInNn('nn');\"><b><u>П</u></b>оказывать названия нот</acronym><br>" +
                  "<acronym title=\"Интервалы: 1, 2, 3, итд.\"><input type=radio name=\"displayNames\" value=\"in\" onClick=\"switchInNn('in');\"><b><u>П</u></b>оказывать интервалы</acronym></span><td>");
   document.write("<td width=180><span id=\"rb_spelling\" class=\"rb\"><acronym title=\"Строгая гармония: двойной диез (x), двойной бемоль (bb), итд.\"><input type=radio name=\"enharmonicSpelling\" value=\"STRICT\" onClick=\"switchSpelling('STRICT');\">Строгая <b><u>г</u></b>армония <font color=\"#666666\">(x = ##)</font></acronym><br>" +
                  "<acronym title=\"Упрощенная гармония: общеупотребительные ноты\"><input type=radio name=\"enharmonicSpelling\" value=\"SIMPLE\" onClick=\"switchSpelling('SIMPLE');\">Упрощенная <b><u>г</u></b>армония</acronym></span></td>");
   document.write("<td><span id=\"rb_root\" class=\"rb\">"+"<acronym title=\"#: полутона как диезы\"><input type=radio name=\"rootSharpOrFlat\" value=\"#\" onClick=\"switchSharpFlat('#');\">Полу<b><u>т</u></b>она: #</acronym><br>" +
                  "<acronym title=\"b: полутона как бемоли\">"+"<input type=radio name=\"rootSharpOrFlat\" value=\"b\" onClick=\"switchSharpFlat('b');\">Полу<b><u>т</u></b>она: b</acronym></span>");
...
function c23(c40,strict_or_simple)
{
   ...
   for( var i=0; i<c20.length; i++ )
      ...
      else alert("Ошибка: неизвестный символ в `interval_adjustment'.\nПожалуйста сообщите об ошибке по адресу webmaster@pripev.ru" );
      }
...
function c15(c14,strict_or_simple)
{
   ...
   else alert("Ошибка: Неизвестный параметр '" + c14 + "' функции `lower_one_half_step'.\nПожалуйста сообщите об ошибке по адресу webmaster@pripev.ru");
   return c14;
}
...
function c16(c14,strict_or_simple)
{
   ...
   else alert("Ошибка: Неизвестный параметр '" + c14 + "' функции `raise_one_half_step'.\nПожалуйста сообщите об ошибке по адресу webmaster@pripev.ru");
   return c14;
}
