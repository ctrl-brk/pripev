...
function up_SetProgress(progress, file)
{
    ...
    html += (progress == 0 ? "Ожидание загрузки" : "Загрузка " + file + " " + progress + "%");
    html += "</div>";
    html += "</div>";
    html += "</div>";
    ...
    if (!up_loading)
    {
        Modalbox.show(html, {title: "Идет загрузка", width: 600, height: 120, aspnet:true});
        Modalbox.deactivate();
    }
    else
    {
...
{  
   ...
   up_waiting = true;
    ...
    if (up_xhttp.status == 200)
    {
        ...
        {
            alert('Превышен максимальный размер файла. Загрузка будет прервана.');
